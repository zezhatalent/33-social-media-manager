...
export default function BackgroundFx() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden" aria-hidden>
      ...
      {/* aurora blobs */}
      <motion.div
        className="absolute -left-32 -top-32 h-[42rem] w-[42rem] rounded-full blur-[120px]"
        style={{ background: `${t.accent}33` }}
        animate={{ x: [0, 80, 0], y: [0, 60, 0], scale: [1, 1.15, 1] }}
        transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute -right-24 top-1/3 h-[36rem] w-[36rem] rounded-full blur-[120px]"
        style={{ background: `${t.accent2}30` }}
        animate={{ x: [0, -70, 0], y: [0, 80, 0], scale: [1.1, 1, 1.1] }}
        transition={{ duration: 26, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        className="absolute bottom-0 left-1/3 h-[30rem] w-[30rem] rounded-full blur-[110px]"
